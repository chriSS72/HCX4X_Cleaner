...
while test $# -gt 0; do
    case "$1" in
        ...
        -c|--check)
            ...
            psql -U postgres hybridity -h localhost -c "SELECT name, setting FROM pg_settings WHERE name='autovacuum';";
            print_header ${bcolors[HEADER]} "" "Dead tuples?";
            psql -U postgres hybridity -h localhost -c "SELECT relname, n_dead_tup FROM pg_stat_user_tables;";
            print_header ${bcolors[HEADER]} "" "Last vacuum date?";
            psql -U postgres hybridity -h localhost -c "SELECT relname, last_vacuum, last_autovacuum FROM pg_stat_user_tables;";
            print_header ${bcolors[HEADER]} "" "Largest tables?";
            # Can run on VM Table, Job and Datastore
            psql -U postgres hybridity -h localhost -c "SELECT current_database() as database, 
        ...
        -v|--vacuum)
            ...
            psql -U postgres hybridity -h localhost -c "VACUUM FULL \"$2\";";
